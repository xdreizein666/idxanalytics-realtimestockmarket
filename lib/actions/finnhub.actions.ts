...
export const searchStocks = cache(
	async (query?: string): Promise<StockWithWatchlistStatus[]> => {
		try {
			...
			// Watchlist state is derived from the session, never from a
			// client-supplied email (that was an IDOR).
			const watchlistSymbols = await getWatchlistSymbols();
		const watchlistSymbolSet = new Set(watchlistSymbols.map(w => w.symbol));
			...
			const mapped: StockWithWatchlistStatus[] = results
				.map((r) => {
					...
					const item: StockWithWatchlistStatus = {
						...
						exchange,
						type,
						isInWatchlist: watchlistSymbolSet.has(upper),
					};
					return item;
				})
				.slice(0, 15);
